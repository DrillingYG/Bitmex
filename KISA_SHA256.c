...
#define ENDIAN_REVERSE_ULONG(dwS)	( (ROTL_ULONG((dwS),  8) & 0x00ff00ff)	\
	...
	#define BIG_D2B(D, B)		*(unsigned int*)(B) = (ULONG)(D)
	#define LITTLE_B2D(B, D)	D = ENDIAN_REVERSE_ULONG(*(unsigned int*)(B))
	#define LITTLE_D2B(D, B)	*(unsigned int*)(B) = ENDIAN_REVERSE_ULONG(D)
#elif defined(LITTLE_ENDIAN)
	#define BIG_B2D(B, D)		D = ENDIAN_REVERSE_ULONG(*(unsigned int*)(B))
	#define BIG_D2B(D, B)		*(unsigned int*)(B) = ENDIAN_REVERSE_ULONG(D)
	#define LITTLE_B2D(B, D)	D = *(unsigned int*)(B)
	#define LITTLE_D2B(D, B)	*(unsigned int*)(B) = (ULONG)(D)
#else
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Transform(unsigned int* Message, unsigned int* ChainVar)
{
...
void SHA256_Process( OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen )
{
	...
	while (uDataLen >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((unsigned int*)Info->szBuffer, Info->uChainVar);
...
void SHA256_Close( OUT SHA256_INFO *Info, IN BYTE *pszDigest )
{
	...
	((unsigned int*)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((unsigned int*)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;
